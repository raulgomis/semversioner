...
class AddChangeCommandTest(CommandTest):
    def test_write_new_change(self) -> None:
        ...
        data = []
        files = sorted(os.listdir(self.next_release_dirname))
        ...
        assert re.search(rf"Successfully created file {re.escape(self.next_release_dirname)}.*\.json", result.output)
...
class ChangelogCommandTest(CommandTest):
    ...
    def test_generate_changelog_with_custom_template(self) -> None:
        ...
        result = command_processor([["changelog", "--template", str(get_file("template_01.j2"))]], self.directory_name)

        assert result.output == read_file("template_01_readme.md")

        result = command_processor([["changelog", "--template", str(get_file("template_02.j2"))]], self.directory_name)

        assert result.output == read_file("template_02_readme.md")

        result = command_processor([["changelog", "--template", str(get_file("template_03.j2"))]], self.directory_name)

        assert result.output == read_file("template_03_readme.md")

        result = command_processor([["changelog", "--template", str(get_file("template_04.j2"))]], self.directory_name)

        assert result.output == read_file("template_04_readme.md")

        result = command_processor(
            [["changelog", "--version", "1.0.0", "--template", str(get_file("template_04.j2"))]], self.directory_name
        )

        assert result.output == fixtures.CHANGELOG_4_TEMPLATE_CURRENT_VERSION_PARTIAL

        result = command_processor([["changelog"]], self.directory_name)
    ...
    def test_generate_changelog_with_custom_not_existent(self) -> None:
        ...
        assert (
            "Error: Invalid value for '--template': 'non-existent-file.j2': No such file or directory" in result.output
        )
...
class ExistingDataTestCase(unittest.TestCase):
    ...
    def test_cli_execution_creating_correct_changeset_files(self) -> None:
        ...
        assert not os.path.isdir(self.next_release_dirname)
        ...
        assert os.path.isdir(self.next_release_dirname)
        assert len(os.listdir(self.next_release_dirname)) == 1

        result = command_processor([["release"]], self.directory_name)
...
class NonEmptyNextReleaseFolder(unittest.TestCase):
    ...
    def test_cli_execution_creating_correct_changeset_files(self) -> None:
        ...
        assert len(os.listdir(self.next_release_dirname)) == 2

        result = command_processor([["release"], ["changelog"]], self.directory_name)
        assert result.output == fixtures.CHANGELOG_6
        assert len(os.listdir(self.next_release_dirname)) == 1
